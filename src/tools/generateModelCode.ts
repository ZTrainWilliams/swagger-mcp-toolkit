...
export const generateModelCode = {
  ...
  description: "Generates TypeScript code for a model from the Swagger definition. Precedence: CLI --swagger-url over swaggerFilePath. Use when asked to generate TypeScript model/types from the schema.",
  inputSchema: {
    type: "object",
    properties: {
      modelName: {
      ...
      swaggerFilePath: {
        type: "string",
        description: "Optional path to the Swagger file. Used only if --swagger-url is not provided. You can find this path in the .swagger-mcp-toolkit file in the solution root with the format SWAGGER_FILEPATH=path/to/file.json."
      }
    },
    required: ["modelName"]
  }
